...
class ModbusTransactionManager(object):
    ...
    def _is_exception_response(self, response):
        if len(response) < 2:
            return False
        # If the response function code byte has the msb set to 1
        # the response is a Modbus Exception.
        elif int(response[1]) > 128:
            return True
        else:
            return False
        ...
        while retries > 0:
            try:
                ...
                while result and expected_response_length and len(result) < expected_response_length:
                    if self._is_exception_response(result):
